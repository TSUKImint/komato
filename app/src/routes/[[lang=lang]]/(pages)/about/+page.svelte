<script>
import Image from "$lib/components/Image.svelte";
</script>

<svelte:head>
	<title>about | komato.world</title>
	<meta property="og:title" content="komato.world" />
	<meta
		property="og:description"
		content="The homepage of Hajar R., an artist & designer in Antwerp, Belgium.."
	/>
	<meta
		property="description"
		content="The homepage of Hajar R., an artist & designer in Antwerp, Belgium.."
	/>
	<meta
		property="og:image"
		content="https://komato.world/android-chrome-512x512.png"
	/>
	<meta property="og:type" content="website" />
	<meta property="og:url" content="https://komato.world/" />
	<meta property="og:site_name" content="komato.world" />

	<!-- Twitter -->
	<meta name="twitter:creator" content="@ncb0_" />
	<meta property="twitter:card" content="summary" />
	<meta property="twitter:title" content="komato.world" />
	<meta
		property="twitter:description"
		content="The homepage of Hajar R., an artist & designer in Antwerp, Belgium."
	/>
	<meta property="twitter:url" content="https://komato.world/" />
	<meta
		property="twitter:image"
		content="https://komato.world/android-chrome-512x512.png"
	/>
</svelte:head>

<article class="narrow">
	<h1>about</h1>

	<p style="margin-top: 0.5ex;">
		I’m Hajar, a multidisciplinary artist based in Antwerp. My work spans digital and traditional art, graphic design, type design and creative writing. I value curiosity and exploring many creative forms.
	</p>

	<h2>availability</h2>
	<p>
		Available for freelance work—flyers, album covers, and more. I’m mainly interested in larger projects rather than one-off art commissions.
	</p>

	<h2>contact</h2>
	<p>
		<a href="mailto:kikkomato@dmc.chat">kikkomato@dmc.chat</a>
	</p>

	<h2>elsewhere</h2>
	<h3>
		<a target="_blank" href="https://patreon.com/kikkomato" class="button"
			><svg
				xmlns="http://www.w3.org/2000/svg"
				xml:space="preserve"
				viewBox="0 0 24 24"
				class="icon"
			>
				<path
					fill="rgb(var(--text-color))"
					d="M22 7.63c0-2.8-2.18-5.1-4.74-5.92a15.12 15.12 0 0 0-10.39.55C3.2 4 2.05 7.8 2 11.58c-.03 3.11.28 11.31 4.9 11.37 3.44.05 3.95-4.38 5.54-6.52 1.13-1.51 2.59-1.94 4.38-2.38 3.08-.77 5.18-3.2 5.18-6.42z"
				/>
			</svg>
			Patreon</a
		>
		<a
			target="_blank"
			href="https://www.pixiv.net/en/users/635262623"
			class="button"
			><svg
				xmlns="http://www.w3.org/2000/svg"
				xml:space="preserve"
				style="enable-background:new 0 0 24 24"
				viewBox="0 0 24 24"
				class="icon"
			>
				<path
					fill="rgb(var(--text-color))"
					d="M20.5 4.4C18.7 2.9 16.2 2 13.3 2 5.8 2 .7 7.7.7 7.7l1.4 2.2s.8.1.4-1.3C2.9 8 3.6 7.1 5 6v15c-.6.2-1.4.5-.9 1h4.1c.5-.5-.3-.9-.8-1v-3.5s2.8 1.1 5.9 1.1c2.7 0 5.1-.8 7-2.2 1.8-1.4 3-3.5 3-5.9 0-2.4-1-4.6-2.8-6.1zm-2.2 10.8a7.6 7.6 0 0 1-5.2 2c-2.4 0-4.3-.4-5.6-1.1V4.9c1.4-1 3.8-1.6 5.6-1.6 2.3 0 4.1.8 5.3 2.1 1.2 1.3 1.9 3 1.9 5 0 1.9-.7 3.5-2 4.8z"
				/>
			</svg>
			Pixiv</a
		>
		<a target="_blank" href="https://www.are.na/hajar-r" class="button"
			><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
				<path
					fill="rgb(var(--text-color))"
					d="m23.77 14.85-3.33-2.61a.37.37 0 0 1 0-.59l3.33-2.61c.2-.16.29-.43.19-.6-.1-.17-.38-.23-.62-.13l-3.96 1.57c-.24.1-.47-.03-.5-.3l-.6-4.21c-.03-.26-.23-.47-.43-.47-.2 0-.39.21-.42.47l-.6 4.22c-.03.26-.26.39-.5.3l-3.9-1.56a1.36 1.36 0 0 0-.87 0L7.67 9.88c-.24.1-.47-.03-.5-.3l-.6-4.21c-.03-.26-.23-.47-.43-.47-.2 0-.39.21-.43.47l-.6 4.22c-.03.26-.25.39-.5.3L.67 8.3c-.24-.1-.52-.04-.62.13-.1.17-.01.44.2.6l3.32 2.61c.2.16.2.43 0 .59L.23 14.85c-.2.16-.29.45-.19.64.1.19.38.26.62.17l3.92-1.58c.24-.1.47.04.5.3l.56 4.25c.04.26.26.47.5.47s.47-.21.5-.47l.57-4.26c.03-.25.26-.38.5-.29l3.85 1.55c.24.1.64.1.88 0l3.85-1.55c.24-.1.47.04.5.3l.57 4.25c.03.26.26.47.5.47.23 0 .46-.21.5-.47l.56-4.26c.03-.25.26-.38.5-.29l3.92 1.58c.24.1.52.02.62-.17.1-.2.01-.48-.2-.64zm-8.47-2.58-2.93 2.25a.64.64 0 0 1-.74 0L8.7 12.27c-.2-.16-.21-.42 0-.58l2.93-2.31c.2-.16.54-.16.74 0l2.94 2.31c.2.16.2.42 0 .58z"
				/>
			</svg>
			Are.na</a
		>
		<a target="_blank" href="https://ko-fi.com/komato" class="button"
			><svg
				xmlns="http://www.w3.org/2000/svg"
				xml:space="preserve"
				viewBox="0 0 24 24"
				class="icon"
			>
				<path
					fill="rgb(var(--text-color))"
					d="M22.6 9.8c-.3-1.5-1.1-2.5-2-3.1-.9-.6-1.9-.9-3-.9H2.5c-.5 0-.7.5-.7.8v10.5c.1 2.1 2.2 2.1 2.2 2.1h10.3c.2 0 .3 0 .5-.1 1.9-.5 2.1-2.2 2.1-3.2 3.7.2 6.4-2.5 5.7-6.1zM13 12.9c-1.1 1.3-3.5 3.5-3.5 3.5s-.1.1-.3 0c-.1 0-.1-.1-.1-.1-.4-.4-2.9-2.7-3.5-3.5-.6-.8-.9-2.3-.1-3.2.8-.9 2.6-.9 3.8.4 0 0 1.4-1.6 3-.8 1.7.6 1.6 2.5.7 3.7zm5.4.4c-.8.1-1.5 0-1.5 0v-5h1c.7 0 1.3.3 1.7.8.3.4.5.8.5 1.5.1 1.7-.8 2.4-1.7 2.7z"
				/>
			</svg>
			Ko-fi</a
		>
	</h3>

	<hr style="clear: none;" />

	<figure class="float-right" style="width: 170px; position: relative; right: 426px; top: -10px;">
  <img src="/img/deer_mirrored.png" alt="Lisa in her mouse-human hybrid form…" style="width: 100%; height: auto;" />
  <figcaption style="text-align: center; font-size: 0.9em;">HAJAR R (C) 2025.</figcaption>
</figure>

	<p>&lt;/&gt; WITH &lt;3 BY LISA M, 2025.</p>

	<h4 id="fonts">Fonts</h4>

	<ul>
		<li>Helvetica</li>
		<!-- <li>
			<a
				href="https://github.com/weiweihuanghuang/fragment-mono"
				target="_blank">Fragment Mono</a
			> (Modified)
		</li> -->
		<li>River Mono (TBA)</li>
		<li><a href="/type#tm-lagrange-mono">tm-lagrange-mono</a></li>
	</ul>

	<h4 id="tools">Tools</h4>

	<ul>
		<li>Visual Studio Code</li>
		<li>Inkscape</li>
		<li>Adobe Illustrator CC</li>
		<li>Clip Studio Paint</li>
		<li><a href="https://kit.svelte.dev" target="_blank">SvelteKit</a></li>
		<li><a href="https://sanity.io" target="_blank">Sanity CMS</a></li>
		<li><a href="https://vercel.com" target="_blank">Vercel</a></li>
	</ul>
</article>
